...
class Fredformer_backbone(nn.Module):
    def __init__(self, ablation:int,  use_nys:int, output:int, cf_dim:int,cf_depth :int,cf_heads:int,cf_mlp:int,cf_head_dim:int,cf_drop:float,c_in:int, context_window:int, target_window:int, patch_len:int, stride:int,  d_model:int, 
        ...
        self.freq_embed_dim = self.fre_transformer.mlp_head.out_features
        ...
        self.time_projection = nn.Linear(self.patch_num * self.freq_embed_dim,target_window)
    ...
    def forward(self, z):                                                                   # z: [bs x nvars x seq_len]
        ...
        z_patch = torch.cat((z1, z2), -1)                                     # [bs, patch_num, nvars, 2*patch_len]
        z_patch = z_patch.view(batch_size * patch_num, c_in, -1)
        z = self.fre_transformer(z_patch)
        embed_dim = z.shape[-1]
        z = z.view(batch_size, patch_num, c_in, embed_dim)                    # [bs, patch_num, nvars, embed_dim]
        z = z.permute(0, 2, 1, 3).contiguous()                                # [bs, nvars, patch_num, embed_dim]
        z = z.view(batch_size * c_in, patch_num * embed_dim)
        z = self.time_projection(z)
        z = z.view(batch_size, c_in, self.targetwindow)
