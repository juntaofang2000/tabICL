...
class Trainer:
    ...
    def build_model(self):
        """Build and initialize the MantisTabICL model."""

        # MantisTabICL requires checkpoints for initialization
        if not hasattr(self.config, 'tabicl_checkpoint') or not self.config.tabicl_checkpoint:
            raise ValueError("MantisTabICL requires --tabicl_checkpoint to be specified.")
        
        print(f"Building MantisTabICL with:")
        print(f"  TabICL Checkpoint: {self.config.tabicl_checkpoint}")
        print(f"  Mantis Checkpoint: {self.config.mantis_checkpoint}")

        model = MantisTabICL(
            tabicl_checkpoint=self.config.tabicl_checkpoint,
            mantis_checkpoint=self.config.mantis_checkpoint,
            mantis_batch_size=self.config.batch_size,
        ...
        if self.master_process:
            ...
            print(f"Total Model has {total_params} trainable parameters.")
            
            if hasattr(model, "mantis_model"):
                print(f"  - Mantis module: {count_params(model.mantis_model)} parameters")
            
            if hasattr(model, "tabicl_model"):
                print(f"  - TabICL module: {count_params(model.tabicl_model)} parameters")
                if hasattr(model.tabicl_model, "icl_predictor"):
                    print(f"    - ICL predictor: {count_params(model.tabicl_model.icl_predictor)} parameters")
    ...
    def configure_prior(self):
        ...
        self.dataloader = DataLoader(
            ...
            shuffle=False,
            num_workers=0,  #  fangjuntao  1-> 0 to avoid nested parallelism during DDP
            prefetch_factor=None, #  fangjuntao  4-> None  to avoid nested parallelism during DDP
    ...
    def save_checkpoint(self, name: str):
        ...
        checkpoint = {
            # "config": self.model_config, # MantisTabICL doesn't use model_config dict in init
    ...
    def run_micro_batch(self, micro_batch, micro_batch_idx, num_micro_batches):
        ...
        with self.amp_ctx:
            # MantisTabICL forward signature: (X, y_train, ...)
            # It does not take 'd' (feature count) as it works on embeddings/raw features handled by Mantis
            pred = self.model(micro_X, y_train)  # (B, test_size, max_classes)
    ...
    def run_batch(self, batch):
        ...
        # Ensure Mantis is in training mode (since we want to fine-tune it)
        # Note: If you want to freeze Mantis, use the freeze arguments or modify the code
        # if hasattr(self.raw_model, "mantis_model"):
        #      self.raw_model.mantis_model.train()
...
if __name__ == "__main__":
    print("Starting MantisTabICL training...")
    parser = build_parser()
    
    # Add arguments specific to MantisTabICL
    parser.add_argument("--tabicl_checkpoint", type=str, default="/data0/fangjuntao2025/tabicl-main/tabICLOrignCheckpoint/tabicl-classifier-v1.1-0506.ckpt", help="Path to pretrained TabICL checkpoint")
    parser.add_argument("--mantis_checkpoint", type=str, default="/data0/fangjuntao2025/CauKer/CauKerOrign/CauKer-main/Models/Mantis/Mantis_cheickpoint/", help="Path to pretrained Mantis checkpoint")
