...
class TabICLClassifier(ClassifierMixin, BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        # Load the pre-trained TabICL model if not already loaded or if model_path changed
...
class MantisICLClassifier(ClassifierMixin, BaseEstimator):
    """Tabular In-Context Learning (TabICL) with scikit-learn interface.
    ...
    def fit(self, X, y):
        ...
        2. Converting input features to numerical values
        3. Fitting the ensemble generator to create transformed dataset views
        4. Loading the pre-trained MantisICL model
        ...
        # Load the pre-trained MantisICL model if not already loaded or if model_path changed
        need_load = not hasattr(self, "model_")
        if not need_load and self.model_path is not None:
            try:
                current_path = Path(self.model_path) if isinstance(self.model_path, str) else self.model_path
    ...
    def _batch_forward(self, Xs, ys, shuffle_patterns=None):
        ...
        This method handles the batched inference through the MantisICL model,
        dividing the ensemble members into smaller batches to avoid out-of-memory errors.
    ...
    def predict_proba(self, X):
        ...
        Applies the ensemble of MantisICL models to make predictions, with each ensemble
        member providing predictions that are then averaged. The method:
        1. Transforms input data using the fitted encoders
        2. Applies the ensemble generator to create multiple views
        3. Forwards each view through the model
        ...
        if self.n_jobs is not None:
            ...
            if self.n_jobs > 0:
                if self.n_jobs > n_logical_cores:
                    warnings.warn(
                        f"MantisICL got n_jobs={self.n_jobs} but there are only {n_logical_cores} logical cores available."
                        f" Only {n_logical_cores} threads will be used."
                    )
                n_threads = min(n_logical_cores, self.n_jobs)
            else:
